...
async def test_create_embedding():
    ...
    embedding_id = await repo.create_embedding(
        attraction_id="eiffel_tower_123",
        embedding=embedding_data,
        review_id="review_23456",
        review_content="Great place to visit! ",
    )
...
async def test_update_embedding_by_review_id(review_id: str, embedding: list[float]):
    client = AsyncIOMotorClient(
        "mongodb+srv://LJC:asasdd@cluster0.gif9hs8.mSongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
    )
    db = client["review_summary_db"]
    repo = ReviewEmbeddingRepository(db)
    result = await repo.update_embedding_by_review_id(
        review_id=review_id, embedding=embedding
    )
    print(result)
    client.close()
