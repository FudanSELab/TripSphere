...
class ReviewSummaryAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            llm_wrapper = ModelWrapper(llm)

            target_name = await llm_wrapper.ainvoke(TARGET_NAME_PROMPT+"\nquery:"+query)

            target_id = find_attraction_id_by_name(target_name)
            ...
            search = LocalSearch(
                model=llm,
                context_builder=contex_builder,
                tokenizer=tokenizer,
            )
