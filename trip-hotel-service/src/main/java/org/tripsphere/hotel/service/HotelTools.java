...
public class HotelTools {
     ...
     */
    @McpTool(name = "findHotelsWithinRadius", description = "Find all hotels within a radius from a location")
    public List<Hotel> findHotelsWithinRadius(@McpToolParam(description = "Longitude", required = true) double lng,
                                              @McpToolParam(description = "Latitude", required = true)double lat,
                                              @McpToolParam(description = "Distance to central point, kilometer", required = true)double radiusKm,
                                              @McpToolParam(description = "Hotel name, If you want all hotels, just don't input anything", required = true)String name,
                                              @McpToolParam(description = "Hotel tags, such like ”餐饮“,”茶室“ and ”洗衣服务“", required = true)List<String> tags) {
