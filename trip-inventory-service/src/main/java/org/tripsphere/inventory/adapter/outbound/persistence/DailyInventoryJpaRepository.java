...
import org.tripsphere.inventory.adapter.outbound.persistence.entity.DailyInventoryEntity;

public interface DailyInventoryJpaRepository extends JpaRepository<DailyInventoryEntity, String> {

    Optional<DailyInventoryEntity> findBySkuIdAndInvDate(String skuId, LocalDate invDate);

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT d FROM DailyInventoryEntity d" + " WHERE d.skuId = :skuId AND d.invDate = :invDate")
