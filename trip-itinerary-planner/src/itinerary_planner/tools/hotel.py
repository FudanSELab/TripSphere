...
class HotelDetail(BaseModel):
    ...
    tags: list[str] = Field(default_factory=list, description="Soft tags (e.g., ['拍照出片', '氛围浪漫'])")
    images: list[str] = Field(default_factory=list, description="Image URLs")
    introduction: str = Field(description="Hotel introduction/description")
    estimated_price: float | None = Field(default=None, description="Estimated price per night, if available")
    amenities: list[str] = Field(default_factory=list, description="Amenities (e.g., ['健身房', '游泳池'])")
...
async def search_hotels_nearby(
    ...
    for hotel in response.hotels[:limit]:
        ...
        if hasattr(hotel, "estimated_price") and hasattr(hotel.estimated_price, "units"):
            try:
                estimated_price = float(hotel.estimated_price.units or 0)
                if hasattr(hotel.estimated_price, "nanos") and hotel.estimated_price.nanos:
        ...
        hotel_details.append(
            HotelDetail(
                ...
                introduction=getattr(hotel, "information", None) and getattr(hotel.information, "introduction", "") or "",
